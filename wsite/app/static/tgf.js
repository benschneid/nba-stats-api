function dropdowns() {
  ...
  var seas_age = {'Seasons': [1946, 2019]}
  ...
  var radios = {"Game_Type": ["Regular Season", "Playoffs", "Either"],
                ...
                "Overtime": ["Yes", "No", "Either"],
                };
  for(var key in radios){
     var fieldset = document.createElement("FIELDSET");
     if(key == "Game_Type" || key == "Overtime"){
        flex.appendChild(fieldset);
     }
     else{
  ...
  for(i = 0; i < 4; i++){
      ...
      for(var key in droppies){
        ...
        for(var subkey in droppies[key]){
            ...
            var res = droppies["stats"][subkey].replace(" ", "_");
            a_item.setAttribute("value", (key == "stats") ? res: "lt");
            }
      }
  ...
  document.getElementById('theform').onsubmit = function(){
    window.location = '/tgf/find?' + $(document.getElementById('theform')).serialize() + "&page=1";
    return false;
  };
